...
function MediaCarousel({ images }: { images: StudentLifeEntry["images"] }) {
  ...
  if (!images?.length) {
    return (
      <div className="h-64 w-full flex items-center justify-center bg-slate-100 rounded-2xl text-muted-foreground">
        Images coming soon
      </div>
    );
  }
  ...
  return (
    <Carousel className="w-full" opts={{ loop: images.length > 1 }} setApi={setCarouselApi}>
      <CarouselContent>
        {images.map((image) => (
          <CarouselItem key={image.id}>
            <div className="h-64 w-full overflow-hidden rounded-2xl bg-slate-100">
              <img src={image.imageUrl} alt="Student life moment" className="h-full w-full object-cover" />
            </div>
          </CarouselItem>
        ))}
      </CarouselContent>
